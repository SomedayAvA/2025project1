...
class VEINS_API TraCIDemo11pMessage : public ::veins::BaseFrame1609_4
{
  protected:
    ...
    LAddress::L2Type senderAddress = -1;
    int serial = 0;
    omnetpp::opp_string s1Ciphertext;
    omnetpp::opp_string d1Ciphertext;
    omnetpp::opp_string publicKeyStr;
    omnetpp::opp_string SValues;
    omnetpp::opp_string DValues;
    std::unique_ptr<pcs_public_key, decltype(&pcs_free_public_key)> pk{nullptr, &pcs_free_public_key};
    std::unique_ptr<hcs_random, decltype(&hcs_free_random)> hr{nullptr, hcs_free_random};
  private:
  ...
  public:
    ...
    virtual const char * getSValues() const;
    virtual void setSValues(const char * demoData);
    virtual const char * getDValues() const;
    virtual void setDValues(const char * demoData);
    virtual const char * getDemoData() const;
    virtual void setDemoData(const char * demoData);
    virtual const char * getS1Ciphertext() const;
    virtual void setS1Ciphertext(const char * demoData);
    virtual const char * getD1Ciphertext() const;
    virtual void setD1Ciphertext(const char * demoData);
    virtual const char * getPublicKeyStr() const;
    virtual void setPublicKeyStr(const char * demoData);
