...
class VEINS_API DamgardJurik : public DemoBaseApplLayer {
public:

    virtual ~DamgardJurik();
    DamgardJurik()
        : hr(nullptr, hcs_free_random),
          pk(nullptr, djcs_free_public_key),
          vk(nullptr, djcs_free_private_key){}
...
private:
    ...
    std::unique_ptr<djcs_public_key, decltype(&djcs_free_public_key)> pk;
    std::unique_ptr<djcs_private_key, decltype(&djcs_free_private_key)> vk;
